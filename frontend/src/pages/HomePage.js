...
function HomePage() {
  ...
  const login = async () => {
    if (email && password) {
      try {
        const response = await axios.post('https://quizz-app-backend-beta.vercel.app/auth/login', { email, password });
  ...
  const register = async () => {
    if (email && password) {
      try {
        await axios.post('https://quizz-app-backend-beta.vercel.app/auth/register', { email, password });
